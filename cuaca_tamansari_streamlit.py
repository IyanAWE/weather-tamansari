...
page = st.sidebar.radio("Pilih Halaman", ["Dashboard Utama", "Data Historis"])
...
# --- Halaman Data Historis ---
    ...
    st.title("📊 Data Historis Cuaca")

    try:
        df_bmkg_historis = ambil_data_bmkg_sheet()
        if not df_bmkg_historis.empty:
            st.dataframe(df_bmkg_historis)
        else:
            st.info("Belum ada data historis di Google Sheets.")
    except Exception as e:
        st.warning(f"⚠️ Gagal ambil data historis BMKG: {e}")
